...
export default function VisitsManagement() {
  ...
  const [selectedDay, setSelectedDay] = useState(null); // { date, visits[] }
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await axiosInstance.get("/calendar/visits", { params: { startDate, endDate } });
  ...
  const visitsByDate = {};
  visits.forEach((m) => {
    if (!m.visitDate) return;
    const key = new Date(m.visitDate).toISOString().split("T")[0];
    if (!visitsByDate[key]) visitsByDate[key] = [];
    visitsByDate[key].push(m);
  });

  // Apply search filter to list view
  const filteredList = visits.filter((m) => {
  ...
  const handleDayClick = (day) => {
    ...
    setSelectedDay({ dateStr, day, visits: dayVisits });
  };
  ...
  if (loading)
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3">
        ...
        <p className="text-sm font-semibold" style={{ color: c.textSecondary }}>Loading visits…</p>
      </div>
    );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black flex items-center gap-2" style={{ color: c.text }}>
            <Calendar size={26} style={{ color: c.primary }} /> Visits
          </h1>
          <p className="mt-1 text-sm" style={{ color: c.textSecondary }}>
            {visits.length} total visits scheduled
          </p>
        </div>
                  ...
                  <div className="space-y-0.5">
                    {dayVisits.slice(0, 2).map((m) => (
                      <div
                        ...
                        title={`${m.name} — ${new Date(m.visitDate).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}`}
                      >
                        {new Date(m.visitDate).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })} {m.name}
                      </div>
                    ))}
                    {dayVisits.length > 2 && (
        ...
        <div className="rounded-2xl border" style={{ backgroundColor: c.surface, borderColor: c.border }}>
          <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: c.border }}>
            <h3 className="font-black text-lg" style={{ color: c.text }}>
              ...
              <span className="ml-2 text-sm font-semibold" style={{ color: c.textSecondary }}>
                ({selectedDay.visits.length} visit{selectedDay.visits.length !== 1 ? "s" : ""})
              </span>
            </h3>
          ...
          {selectedDay.visits.length === 0 ? (
            <div className="py-10 text-center" style={{ color: c.textSecondary }}>
              ...
              <p className="text-sm">Is din koi visit nahi hai</p>
            </div>
          ) : (
            <div className="divide-y" style={{ borderColor: c.border }}>
              {selectedDay.visits.map((m) => (
                <div key={m._id} className="px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex flex-col items-center justify-center rounded-xl px-4 py-3 min-w-[110px] text-center"
                    ...
                    <div className="flex items-center gap-1 text-xs font-semibold mb-1" style={{ color: c.primary }}>
                      <Calendar size={12} />
                      {new Date(m.visitDate).toLocaleDateString("en-IN", {
                        day: "numeric", month: "short", year: "numeric",
                      })}
                    </div>
                    <div className="flex items-center gap-1 text-sm font-black" style={{ color: c.primary }}>
                      <Clock size={13} />
                      {new Date(m.visitDate).toLocaleTimeString("en-IN", {
        ...
        <div className="flex flex-col gap-4 flex-1 overflow-y-auto">
          ...
          {filteredList.length === 0 ? (
            <div className="py-20 text-center rounded-2xl border"
              ...
              <p className="font-bold text-lg" style={{ color: c.text }}>No visits found</p>
