...
const Sidebar = ({
  ...
  // Active check — handles dynamic :id routes too
  const isRouteActive = (route) => {
    if (route.children) return route.children.some(child => isRouteActive(child));
    const basePath = route.path.includes(":") ? route.path.split("/:")[0] : route.path;
    if (currentPath === route.path) return true;
    if (basePath && basePath !== "/" && currentPath.startsWith(basePath + "/")) return true;
    return false;
  };
